...
func run() -> void:
	...
	for frame in (80 if field else 250):
		if frame==45 and not field:
		...
		if frame==35 or (reaction_frame>=0 and frame in [reaction_frame+8,reaction_frame+24,reaction_frame+60]) or (field and frame==79):
	...
	print("ELEMENT CAPTURE: field=",field," narrow=",narrow," reaction_frame=",reaction_frame," peak_steam=",peak," dropped=",game.arena.steam.dropped)
