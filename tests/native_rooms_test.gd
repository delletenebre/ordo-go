...
func _init() -> void:
	...
	for result in ["win","lose"]:
		receive(4,{"type":"start"})
		check(server.rooms[match_code].started,"Native room starts a match")
		receive(4,{"type":"snapshot","state":{"phase":result}})
		receive(5,{"type":"lobby"})
		check(server.rooms[match_code].started,"Guest cannot return everyone to the lobby")
		messages.clear();receive(4,{"type":"lobby"})
		check(not server.rooms[match_code].started and server.peers[5].room==match_code,"Native rematch keeps room and peers")
		check(messages.filter(func(m):return m.data.type=="lobby").size()==2,"Both peers return to waiting")
		messages.clear();receive(5,{"type":"command","slot":1,"data":{"action":"ready","turn":1}})
		check(not messages.any(func(m):return m.data.type=="command"),"Lobby rejects old match input")
		receive(5,{"type":"avatar","slot":1,"avatar":"bugu"})
		check(server.peers[5].avatars==["bugu"],"Avatar can change between games")
	receive(4,{"type":"start"});server.remove_peer(5)
	check(server.rooms.has(match_code) and not server.rooms[match_code].started,"Guest departure preserves native room")
	server.add_peer(6);receive(6,{"type":"join","code":match_code,"count":1})
	check(server.peers[6].room==match_code,"Replacement player uses the same room code")
	receive(4,{"type":"leave"})
	check(not server.rooms.has(match_code) and server.peers[6].room=="","Only host departure closes native room")
