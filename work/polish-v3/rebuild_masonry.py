...
p=Path('scripts/masonry.gd')
s=p.read_text()
a=s.index('# Bevelled'); b=s.index('static func hearth')
s=s[:a]+'''# Shared corner positions keep each chipped block closed, including its bevels.
...
'''+s[b:]
a=s.index('static func wall')
s=s[:a]+'''static func wall(arena) -> void:
...
static func lantern(arena, p: Vector3, index: int) -> void:
	...
	var iron := arena.material(Color("28201c"));iron.metallic=.72;iron.roughness=.58
	var bronze := arena.material(Color("81532d"));bronze.metallic=.75;bronze.roughness=.47
