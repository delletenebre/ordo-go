...
func build(arena, r: float, id: int) -> void:
	...
	var dark=arena.wool(Color("171522"));var light=arena.material(Color("ffc46c"),0.7)
	for side in [-1.0,1.0]:
...
func step(dt: float, look: Vector2) -> void:
	...
	var eye_scale:=Vector2.ONE;var mouth_size:=Vector2(0.33,0.07);var brow_angle:=0.0;var brow_raise:=0.0
	match mood:
	...
	for i in 2:
		...
		brows[i].position.y=lerpf(brows[i].position.y,radius*(1.39+brow_raise),blend)
