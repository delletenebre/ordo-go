...
func make_world() -> void:
	...
	settings.ambient_light_color = Color("a3bbd4"); settings.ambient_light_energy = 0.30
	...
	camera.position = Vector3(0, 15.5, 14.0); add_child(camera); camera.look_at(Vector3(0, 0, 0.6))
	camera.current = true
	...
	for i in 6:
		var a := TAU*i/6.0+0.3
		Masonry.lantern(self, Vector3(cos(a)*6.48,0.48,sin(a)*6.48),i)
	Masonry.hearth(self)
...
func flame_shape(size: float) -> ArrayMesh:
	var st := SurfaceTool.new(); st.begin(Mesh.PRIMITIVE_TRIANGLES)
	var profile := [Vector2(0.10,0),Vector2(0.27,0.22),Vector2(0.23,0.55),Vector2(0.105,0.93),Vector2(0,1.46)]
	for j in 4:
		for i in 7:
			for corner in [Vector2i(i,j),Vector2i(i+1,j),Vector2i(i+1,j+1),Vector2i(i,j),Vector2i(i+1,j+1),Vector2i(i,j+1)]:
				var t: float = profile[corner.y].y; var a: float = corner.x * TAU/7.0
				var radius: float = profile[corner.y].x * size
				st.set_uv(Vector2(float(corner.x)/7, t/1.46)); st.set_smooth_group(-1)
				st.add_vertex(Vector3(cos(a)*radius+sin(t*2.2)*t*size*0.07,t*size,sin(a)*radius))
...
func make_flame(p: Vector3, size: float) -> Node3D:
	...
	mat.set_shader_parameter("tint", Color("ff791e")); mat.set_shader_parameter("heat", 0.85)
	...
	inside.set_shader_parameter("tint", Color("ffe4a0")); inside.set_shader_parameter("heat", 1.35)
	var inner := mesh_node(root, flame_shape(size * 0.7), Vector3(0, 0.0, size * 0.21), inside)
	var tip := mesh_node(root, flame_shape(size * 0.65), Vector3(size * 0.22, 0, 0), mat)
	tip.rotation.z = -0.25
...
func create_actor(e: Dictionary, is_player: bool) -> Node3D:
	...
	if is_player:
		...
		for side in [-1,1]:
			sphere(root,Vector3(side*r*0.52,0.08,0.1),Vector3(r*0.55,r*0.3,r*0.7),wool(color.darkened(0.12)))
...
func render_state(sim, dt: float, remote: bool = false) -> void:
	...
	for flame in flames:
		var t: float = clock * 2.1 + float(flame.offset)
		flame.root.scale = Vector3(1.0 + sin(t) * 0.015, 1.0 + cos(t * 1.3) * 0.025, 1.0 + sin(t + 1.0) * 0.015)
		flame.tip.rotation.z = sin(t * 0.7) * 0.045
	...
	fire_light.light_energy = (2.7 + sin(clock * 2.6) * 0.055) * maxf(0.1, float(sim.fire) / maxf(1.0, float(sim.max_fire)))
...
func play_event(e: Dictionary) -> void:
	...
	if sound_map.has(kind):sound(sound_map[kind])
...
func sound(name_value: String) -> void:
	if not muted and audio_system:audio_system.play_sound(name_value)
